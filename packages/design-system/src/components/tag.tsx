...
export const Tag = ({ variant = 'points', hasBackground = true, icon, text, className, ...rest }: TagProps) => {
  const baseStyles = 'flex flex-row items-center gap-[6px] px-[6px] h-6 w-fit text-sm leading-none flex-shrink-0';

  const variants: Record<Variant, string> = hasBackground
    ? {
        ...
        payouts: 'bg-[var(--color-accent-purple-bg)] text-[var(--color-primary-accent-purple)] rounded-[6px]',
      }
    : {
        points: ' text-[var(--color-primary-accent-purple)] rounded-[6px]',
        'bounty-dark': '  text-[var(--color-secondary-white)] rounded-[6px]',
        active: '  text-[var(--color-primary-text-dark)] rounded-[6px]',
        'profile-points': '  text-[var(--color-primary-text-dark)] rounded-full',
        'dark-icon': '  text-[var(--color-secondary-white)] rounded-[6px]',
        'mid-icon': '  text-[var(--color-primary-text-dark)] rounded-[6px]',
        going: '  meta-title rounded-[34px] gap-[10px] px-[6px] h-[13px] w-[42px]',
        skill: 'border border-[var(--color-secondary-medium-grey)] text-[var(--color-primary-text-dark)] rounded-[6px]',
        payouts: 'text-[var(--color-dark-text)] rounded-[6px]',
      };
  ...
  return (
    <div className={clsx(baseStyles, variants[variant], className)} {...rest}>
      ...
      {text && <span className="flex items-center tag-text">{text}</span>}
